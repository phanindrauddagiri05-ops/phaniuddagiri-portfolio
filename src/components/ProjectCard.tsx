...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project }) => {
  return (
    <div className="bg-white border border-[#E5EAF2] rounded-2xl overflow-hidden card-shadow card-shadow-hover flex flex-col group h-full">
      {/* Thumbnail Container */}
      <div className="relative aspect-[16/10] overflow-hidden bg-slate-900 border-b border-slate-100">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-[#080F1D]/80 via-transparent to-transparent opacity-90 sm:opacity-0 sm:group-hover:opacity-100 transition-opacity duration-300 flex items-end p-4">
          <span className="text-xs font-semibold text-white/90 bg-white/10 backdrop-blur-md px-2.5 py-1 rounded-md sm:hidden">
            Tap to view details
          </span>
        </div>
      </div>
      ...
      <div className="p-5 sm:p-6 flex flex-col flex-grow justify-between space-y-4">
        <div className="space-y-2">
          {/* Title */}
          <h3 className="text-lg sm:text-xl font-bold text-[#101828] group-hover:text-[#4F7CFF] transition-colors leading-snug">
            {project.title}
          </h3>
        ...
        <div className="flex flex-wrap gap-1.5 sm:gap-2 pt-1">
          {project.technologies.map((tech) => (
            <span
              key={tech}
              className="px-2.5 py-0.5 sm:px-3 sm:py-1 rounded-full text-[11px] sm:text-xs font-semibold bg-[#F7F9FC] text-[#4F7CFF] border border-[#E5EAF2]"
            >
              {tech}
            </span>
          ))}
