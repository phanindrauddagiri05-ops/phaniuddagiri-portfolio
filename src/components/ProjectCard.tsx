...
export const ProjectCard: React.FC<ProjectCardProps> = ({ project, index }) => {
  return (
    <div className="bg-white border border-slate-200/90 hover:border-blue-500/50 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl hover:shadow-blue-500/10 transition-all duration-300 transform hover:-translate-y-1 flex flex-col group h-full relative">
      ...
      >
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-slate-950/80 via-slate-950/20 to-transparent opacity-70 group-hover/img:opacity-50 transition-opacity duration-300" />
        ...
        {/* Floating Quick Action Overlay on Desktop Hover / Touch Tap */}
      ...
      <div className="p-4 sm:p-5 flex flex-col flex-grow justify-between space-y-3.5">
        ...
        <div className="flex flex-wrap gap-1.5 pt-1">
          {project.technologies.map((tech) => (
            <span
              key={tech}
              className="px-2.5 py-0.5 rounded-md text-[10px] sm:text-[11px] font-semibold bg-slate-100 text-slate-700 border border-slate-200/80 group-hover:bg-blue-50 group-hover:text-blue-700 group-hover:border-blue-200/80 transition-colors"
            >
              {tech}
            </span>
          ))}
        ...
        {/* Card Footer Links (Mobile Touch Friendly) */}
        <div className="flex items-center justify-between gap-2 pt-3 border-t border-slate-100 mt-auto">
          <a
            href={project.demoUrl}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1 px-3 py-1.5 rounded-full text-xs font-bold text-white bg-blue-600 hover:bg-blue-700 active:scale-95 shadow-sm transition-all"
          >
            <span>Live Demo</span>
            <ArrowUpRight className="w-3.5 h-3.5" />
          </a>
          ...
          {project.githubUrl && (
            <a
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold text-slate-700 bg-slate-100 hover:bg-slate-200 hover:text-slate-900 border border-slate-200 active:scale-95 transition-all"
            >
